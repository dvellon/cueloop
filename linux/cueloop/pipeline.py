...
class NullCueSink:
    ...
    def set_muted(self, muted: bool) -> None:
        self.last_command = {"command": "mute", "muted": muted}
...
class CueLoopPipeline:
    ...
    def feedback(
        self, event_id: str, action: str, *, notify_sink: bool = True
    ) -> bool:
    ...
    def mute(self, seconds: float) -> dict[str, object]:
        self.engine.mute(seconds)
        self.cue_sink.set_muted(seconds > 0)
        return self.engine.state_snapshot()

    def resync_cue_output(self) -> None:
        """Restore MCU output after its restart without changing event history."""

        decision = self.engine.state_snapshot()
        remaining = decision["muted_seconds_remaining"]
        muted = isinstance(remaining, (int, float)) and remaining > 0
        self.cue_sink.set_muted(muted)
        with self._lock:
            current_event = self._current_event
        if (
            not muted
            and current_event is not None
            and current_event.user_action == "pending"
        ):
            self.cue_sink.alert(current_event)
