...
1. Photograph and record exact UNO Q, XIAO Sense, cell, pigtail, cable/power accessory, tool, and optional-part identities in `HARDWARE_RESULTS.md`.
2. On Windows, follow `user_checklists/WINDOWS_FLASHING.md`: UNO Q Blink, App Lab import/Run, XIAO flash with the battery detached, tab-separated NVS configuration, and `TEST ON` transport.
...
4. Enter observations into `HARDWARE_RESULTS.md`; preserve failures and exact conditions.
...
1. Run `user_checklists/PHYSICAL_VALIDATION.md`, including provenance-complete audio splits, per-class quality, end-to-end latency, loss/range/door conditions, recovery, 30-minute soak, battery/runtime/thermal, fit, and accessibility checks.
...
Return the filled `HARDWARE_RESULTS.md` blocks, safe photos/logs or their paths, Fusion error text/export confirmation, and any App Lab/flash error exactly as observed. With those results, thresholds, supported classes, instructions, BOM, CAD dimensions, and final copy can be revised without guessing.
