...
5. In the App Lab monitor, retain the line beginning `CueLoop runtime:`. Expected values for the audited runner family are Python 3.13, an AArch64/ARM64 machine identity, and `ai-edge-litert=2.2.0`; record the actual libc rather than assuming it. Then look for the dashboard/TCP startup text and no model checksum/tensor error. Record all errors verbatim without credentials.
